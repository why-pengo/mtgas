...
from ..exceptions import LogParseError, InvalidLogFormatError, IncompleteLogError
...
class MTGALogParser:
    ...
    PATTERNS = {
        ...
        'timestamp_line': re.compile(r'\[UnityCrossThreadLogger\](\d+/\d+/\d+\s+\d+:\d+:\d+\s+[AP]M)'),
    ...
    def __init__(self, log_path: str):
        ...
        try:
            ...
            raise InvalidLogFormatError(
                "Log file encoding error",
                details=f"Cannot decode file: {e}"
            )
