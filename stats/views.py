...
def dashboard(request):
    ...
    format_stats = (
        Match.objects
        .filter(result__isnull=False, event_id__isnull=False)
        .values('event_id')
        .annotate(
            games=Count('id'),
...
def matches_list(request):
    ...
    formats = Match.objects.exclude(event_id__isnull=True).values_list('event_id', flat=True).distinct()
...
def match_detail(request, match_id):
    ...
    deck_cards = []
    if match.deck:
        deck_cards = match.deck.deck_cards.select_related('card').order_by('card__cmc', 'card__name')
...
def deck_detail(request, deck_id):
    ...
    matchups = (
        deck.matches
        .filter(result__isnull=False, opponent_name__isnull=False)
...
def api_stats(request):
    ...
    daily_stats = (
        Match.objects
        .filter(result__isnull=False, start_time__gte=thirty_days_ago)
        .annotate(date=TruncDate('start_time'))
        .values('date')
        .annotate(
            games=Count('id'),
